...
public class SimpleTimestampFormat extends SimpleDateFormat {
  ...
  /**
   * The pattern string of this formatter. This is always a non-localized pattern. May not be null.
   ...
   * </code> part part of the <code>Timestamp</code>.
   */
  ...
  /**
   * Constructs a <code>SimpleTimestampFormat</code> using the given pattern and the default date
   * format symbols for the default locale. <b>Note:</b> This constructor may not support all
   * locales. For full coverage, use the factory methods in the {@link SimpleTimestampFormat} class.
   *
   ...
   * For full coverage, use the factory methods in the {@link SimpleTimestampFormat} class.
   *
   * @param pattern the pattern describing the date and time format
  ...
  public StringBuffer format(Date timestamp, StringBuffer toAppendTo, FieldPosition pos) {
    ...
    if (timestamp instanceof Timestamp tmp) {
  ...
  /**
   * See <code>SimpleDateFormat</code> description. This is dummy method to deprecate using parent
   * implementation for <code>Timestamp</code> until it is not fully implemented.
   */
  @Override
   ...
   * starting point for the next call to this method. If an error occurs, then the index of <code>
   ...
   * value of the parsed {@code Date} is 1970 with {@link java.util.GregorianCalendar} if no year
   * value is given from the parsing operation. The {@code TimeZone} value may be overwritten,
   * depending on the given pattern and the time zone value in {@code text}. Any {@code TimeZone}
   * value that has previously been set by a call to {@link #setTimeZone(java.util.TimeZone)
