...
NOTE: You'll notice in the script that we wait until the pipeline has finished. It is possible to read batches of data from a stream file, just not the whole file as described in the example.  An alternative is touse an xref:metadata-types/data-stream/arrow-file-stream.adoc[Apache Arrow Flight] data stream type.
